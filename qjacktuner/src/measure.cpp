...
static void smbPitch(long numSampsToProcess, long fftFrameSize, long osamp, float sampleRate, float *indata)
/*
...
*/
{
	...
	long i,k, qpd, // index, 
	  inFifoLatency, stepSize, fftFrameSize2, fadeZoneLen;
...
//		outdata[i] = gOutFIFO[gRover-inFifoLatency];
		...
		if (gRover >= fftFrameSize) {
			...
			ValueEvent *V = ValueEvent::TheValueEvent;
			ValueEvent::TheValueEvent = 0;
...
int initMeasurement()
{
  ...
  return 0;//Decimator.Initialize(DECIMATE_BY, (float*)NULL) ? 0 : -1;
}


void MeasurePitch(long numSampsToProcess, float sampleRate, float *indata)
{
//  static float Buffer[MAX_FRAME_LENGTH/DECIMATE_BY];
//   Decimator.Put(indata, numSampsToProcess);
//   if (Decimator.Get(Buffer, numSampsToProcess / DECIMATE_BY))
//       smbPitch(numSampsToProcess / DECIMATE_BY, 512// 1024
// 	       , 4, sampleRate / DECIMATE_BY, Buffer);
  float *Buffer = Decimator.Dezimiere(indata, numSampsToProcess);
  smbPitch(numSampsToProcess / DECIMATE_BY, 512, 4, sampleRate / DECIMATE_BY, Buffer);
}
